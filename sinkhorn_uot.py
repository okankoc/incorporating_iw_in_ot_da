...
def mm_unbalanced(a, b, C, rho, rho2, numItermax=1000, G0=None, autograd_at_convergence=False):
    ...
    for i in range(numItermax):
        Gd = (torch.sum(G, 1, keepdims=True)**r1) * (torch.sum(G, 0, keepdims=True)**r2) + 1e-16
        G = K * G**(r1 + r2) / Gd
    if autograd_at_convergence is True:
