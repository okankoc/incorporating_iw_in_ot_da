...
class JDOT:
    ...
    def adapt(self, model, fabric, X_source, y_source, X_target, y_target=[]):
        prob, w_dist = self.transport(model, X_source, X_target, y_source, fabric.device)
    ...
    def calc_loss(self, model, X_train, y_train, X_shift, prob_mat):
        ...
        layer_loss = torch.sum(probs * torch.dist(model.features[idx_target], source_activations[idx_source]))
        losses = self.loss_fun(y_pred[idx_target], y_train[idx_source], reduction='none')
    ...
    def calc_loss_mat(self, y_source, y_pred):
        ...
        loss_mat = self.loss_fun(yt, ys, reduction='none').reshape(num_source, num_target)
        return loss_mat
