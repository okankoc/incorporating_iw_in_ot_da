...
class WRR:
    ...
    def propagate_label(self, pred_source, pred_target, fabric):
    ...
    def adapt(self, model, fabric, X_source, y_source, X_target, y_target=[]):
        ...
        if self.propagate_labels is True:
            reg_loss = self.propagate_label(pred_source, pred_target, fabric)
            ...
            return torch.sum(ot_mat * ultra_dist_mat)
        ...
        loss = self.scale * source_loss + reg_loss
